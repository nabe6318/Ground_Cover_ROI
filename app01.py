...
defaults = {
    ...
    "last_uploaded_name": None, "cropped_roi_image": None
}
...
resize_option = st.sidebar.checkbox("画像を縮小して表示・解析", value=True)
max_size = st.sidebar.number_input("表示用最大辺ピクセル数", 600, 5000, 1800, 100)

st.sidebar.markdown("---")
st.sidebar.subheader("縮尺・グリッド設定")
known_dist = st.sidebar.number_input("既知の長さ (m)", 0.001, 100.0, 1.0, format="%.3f")
grid_size_m = st.sidebar.number_input("グリッドサイズ (m)", 0.05, 10.0, 1.0, format="%.2f")
...
if uploaded_file:
    ...
    rgb_disp, _ = resize_if_needed(rgb_orig, max_size) if resize_option else (rgb_orig.copy(), 1.0)
    ...
    if st.session_state.cropped_roi_image is not None:
        ...
        if method == "ExG":
            exg_t = st.sidebar.slider("ExG 閾値", -1.0, 1.0, 0.10, 0.01)
            mask = make_mask_exg(cropped_rgb, exg_t)
        elif method == "HSV(HSB)":
            h_min = st.sidebar.slider("Hue 最小", 0, 255, 30); h_max = st.sidebar.slider("Hue 最大", 0, 255, 90)
            mask = make_mask_hsv(cropped_rgb, h_min, h_max, 50, 40)
        else:
            vari_t = st.sidebar.slider("VARI 閾値", -1.0, 1.0, 0.00, 0.01)
